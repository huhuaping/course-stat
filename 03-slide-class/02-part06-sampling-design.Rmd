...
library("SDAResources") # Lu and Lohr, 2021
library("sampling") # Tillé and Matei, 2021
library("survey") #Lumley, 2020
```
...
```{r}
include_graphics(here("pic/chpt02-kish-table.png"), dpi = 140)
```
...
```{r}
files <- dir(here("data/student-list"))
dir_list <- str_c(here("data/student-list"),"/", files)
...
```{r}
kish <- read.xlsx(here("data/kish-table.xlsx"))
...
**任务**：用**系统抽样**方法从`r n`名学生中随机抽取`r ns`人：
...
- 第三步，确定**抽样距**
$k = \frac{N}{n}=\frac{`r n`}{`r ns`}=`r k_acr` \simeq `r k`$。

- 第四步，把抽样编号排列成一个循环圈，在
$[1,`r k`]$上选择一个随机起点编号
$R$，以
$R$为起点每隔`r k`抽取一个单位，直到抽取总数为`r ns`个单位。
...
students %>%
  select(-class, -grade) %>%
  filter(`序号` %in% spl_sys1) %>%
  datatable(
    caption = "学生名单系统抽样结果（未超出列表）",
    options = list(pageLength = 8)
    ) 
```
...
students %>%
  select(-class, -grade) %>%
  filter(`序号` %in% spl_sys2_final) %>%
  datatable(
    caption = "学生名单系统抽样结果（超出列表）",
    options = list(pageLength = 8)
    ) 
```
...
```{r}
include_graphics(here("pic/chpt02-stratified-sampling.png"),dpi=160)
```
...
```{r}
# source: Lu Y, Lohr S L. R companion for sampling: design and analysis[M]. 第3版. Boca Raton: CRC Press, Taylor & Francis Group, 2022. chpt 3.1 Allocation Methods

data(agpop) # data set from `SDAResource`
agpop <- agpop %>% 
  mutate(county = str_to_title(county)) %>%
  mutate(id = 1:nrow(.)) %>%
  select(id, county, state, region, everything())
...
agpop %>%
  .[, 1:8] %>%
  datatable(
    caption = paste0("地区数据集(N=", N,")"),
    options = list(
      dom = "tip", pageLength = 6,
...
```{r}
al_prop %>%
  janitor::adorn_totals(name = "合计") %>%
  datatable(
    caption = "按地区计算占比及分配数量",
    options = list(dom = "t")
  ) %>%
  formatRound(columns = c(3,4), digits = c(4,0))
```
...
agpop %>%
  .[, 1:8] %>%
  filter(id %in% spl_prop$id_selected) %>%
  datatable(
    caption = paste0("地区成比例抽样数据集(n=", n,")"),
    options = list(
      dom = "tip", pageLength = 6,
      scrollX = TRUE, scrollY = TRUE)
  )
...
al_var %>%
  ...
  janitor::adorn_totals(name = "合计") %>%
  datatable(
    caption = "（考虑方差）按地区计算占比及分配数量",
    options = list(dom = "t")
  ) %>%
  formatRound(columns = c(3,4), digits = c(4,1))
```
...
```{r}
al_cost %>%
  select(-propalloc, -varalloc) %>%
  janitor::adorn_totals(name = "合计") %>%
  datatable(
    caption = "（考虑成本）按地区计算占比及分配数量",
    options = list(
      dom = "t",
      scrollX = TRUE
      )
...
```{r}
al_cost %>%
  select(region, popsize, prop,
         var, relcost, 
         propalloc, varalloc, costalloc) %>%
  janitor::adorn_totals(name = "合计") %>%
  datatable(
    caption = "三种数量分配方案的综合比较",
    options = list(
      dom = "t",
      scrollX = TRUE
      )
  ) %>%
  formatRound(columns = c(3:5), digits = c(4,1,1))

```

> **提问**：数量区别大么？有什么要注意的？三种方案不同，其本质是什么？（更具代表性的样本）
...
```{r}
include_graphics(here("pic/chpt02-cluster-sampling.png"), dpi=300)
```
...
```{r}
file_tar <- "../pic/chpt02-clusting-compare01-stage-psu.png"
include_graphics(file_tar)
```
...
```{r}
file_tar <- "../pic/chpt02-clusting-compare02-stage-psu-srs.png"
include_graphics(file_tar)
```
...
```{r}
file_tar <- "../pic/chpt02-clusting-compare03-stage-ssu-all.png"
include_graphics(file_tar)
```
...
```{r}
file_tar <- "../pic/chpt02-clusting-compare03-stage-ssu-srs.png"
include_graphics(file_tar)
```
...
```{r}
# source： @lohr2021 chpt 05 Cluster Sampling with Equal Probabilities
data(gpa, package = "SDAResources")
ns <- nrow(gpa)

gpa_wide <- gpa %>%
  select(-wt) %>%
  group_by(suite) %>%
  mutate(student = row_number(suite)) %>%
  ungroup() %>%
...
```{r}
# source： @lohr2021 chpt 5.3 two stage cluster sampling

data(schools, package = "SDAResources")
ns <- nrow(schools)
schools_math <- schools %>%
  select(-starts_with("read"), -finalwt)

smry_math <- schools_math %>%
  select(schoolid, Mi) %>%
  group_by(schoolid) %>%
  summarise(
    Mi = mean(Mi),
...
schools_math %>%
  datatable(
    caption = paste0("第二阶段抽样信息(样本数=",ns,")"),
    options = list(
      dom = "tip",
      pageLength = 8
    )
...
```{r data-classes}
data(classes, package = "SDAResources")
N <- nrow(classes)
NN <- sum(classes$class_size)

classes_cum <- classes %>%
  mutate(prop = class_size/sum(class_size)) %>%
  mutate(
    cum_rht = cumsum(class_size) ,
    cum_lft = dplyr::lag(cum_rht, default = 0) +1 )%>%
  mutate(cum_bin = str_c(cum_lft, " - ", cum_rht)) %>%
  mutate(
    index = map2(.x = cum_lft, 
                 .y = cum_rht,
                 ~unlist(seq(.x,.y))
                 )
    ) 
...
n <- 5
k <- NN/n
k_int <- ceiling(k) # 取顶整数
R <- sample(1:k_int, size = 1) # 随机起点
spl_sys <- R + (0:(n-1))*k_int # 随机编号
dt_sys <- classes_cum %>%
  # 匹配随机编号
  mutate(
    spl_true = map(
      .x = index,
      ~ifelse(spl_sys %in% unlist(.x), TRUE, FALSE))
  ) %>%
  # 根据编号确定选定的样本
  mutate(
    selected = map(
    ...
    index_ssu = map(
      .x = spl_true,
      ~unlist(spl_sys[which(unlist(.x)==TRUE)])
    )
  ) %>%
  # 对应找到psu的编号
  mutate(
    psu = map2(
      .x = selected,
      .y = class,
      ~ifelse(.x>0, rep(.y, .x), NA))
  )

psus_id <- which(!is.na(unlist(dt_sys$psu)))
(psus <- unlist(dt_sys$psu)[psus_id])
```
...
```{r}
classes_cum %>%
  #mutate(prop = round(prop, 6)) %>%
  select(class, class_size, prop, cum_bin) %>%
  janitor::adorn_totals(name = "合计") %>% 
  datatable(
    caption = "（总体）班级数据信息",
    options = list(dom = "tip", pageLength = 8)
  ) %>%
    formatRound(columns = 3, digits = 6)
```
...
- 对次级抽样单位进行系统随机抽样：抽样矩
$k=\frac{\sum^N_{i=1}{M_i}}{n}=\frac{`r NN`}{`r n`} = `r k`  \simeq `r k_int`$。在
$[1, `r k_int`]$区间获得随机抽样起点
$R = `r R`$；最后得到系统抽样编号
$\{R, R+k, \cdots, R+(n-1)\times k\} = \{`r spl_sys`\}$

- 最后，根据次级抽样单位（学生编号）的系统抽样结果
$\{`r spl_sys`\}$，确定对应的初级抽样单位（班级编号）的抽样编号
$\{`r psus`\}$。
...
dt_sys %>%
  filter(!is.na(psu)) %>%
  select(class, class_size, prop, cum_bin, index_ssu) %>%
  janitor::adorn_totals(name = "合计") %>% 
  datatable(
    caption = "成比例抽样结果（系统抽样法）",
    options = list(dom = "t")
  ) %>%
    formatRound(columns = 3, digits = 6)
```
...
```{r}
classes_cum %>%
  select(class, class_size, prop) %>%
  .[pps_units,] %>%
  janitor::adorn_totals(name = "合计") %>% 
  datatable(
    caption = "依班级规模概率pps抽样结果",
    options = list(dom = "t")
  ) %>%
  formatRound(columns = 3, digits = 6)
...
dt_reject <- tibble(
  ...
  mutate(
    decision = ifelse(
      choose == TRUE, "成功选定", "不成功，重来")
    ) %>%
  mutate(
    index = 1:nrow(.),
    step = str_c("第", index,"次")
    )
...
```{r, ref.label="script-reject", echo=TRUE}
...
```{r}
dt_reject %>%
  select(step, everything(), -index) %>%
  datatable(
    caption = "Lahiri拒绝法pps抽样过程",
    options = list(
      dom = "t",
      pageLength = 10
    )
...
$\{`r id_choose`\}$。因此最终`Lahiri`拒绝法的抽样结果如下：
...
我们得到了初级抽样单位（班级）的编号为
$\{`r pps_wor_units`\}$。因此最终**系统抽样法**下**无放回**成比例抽样结果如下：

```{r}
dt_pps_wor <- getdata(data = classes, m = spl_pps_wor) %>%
  select(class, class_size, Prob) %>%
  rename("prop" = "Prob")
...
classes_long <- classes %>%
  mutate(
    students = map(class_size, ~tibble(student_id = 1:.x))
  ) %>%
  unnest(students)
```
...
classes_long %>%
  datatable(
    caption = "增加学生信息后的班级案例数据集",
    options = list(
      dom = "tip", pageLength = 8)
  )
```
...
dt_st1 %>%
  janitor::clean_names() %>%
  select(class, class_size, student_id, 
         id_unit, prob_1_stage) %>%
  ...
  formatRound(columns = ~prob_1_stage, digits = 5)
```

> 第一阶段抽中的班级是
$\{ `r dt_st1 %>% select(class)%>% unique() %>% pull("class")` \}$
...
dt_st2 %>%
  ...
  formatRound(columns = ~prob_2_stage+prob, digits = 5)
```
...
```{r, ref.label="classes-complex", echo=TRUE}
...
```{r}
smry_complex <- classes_complex %>%
  nest(.by = strat) %>%
  mutate(
    id_class = map(
      data, ~unlist(unique(.x$class))
    ),
    num_class = map(id_class, ~length(.x)),
    num_students = map(
      data, ~nrow(.x)
    )
  ) %>%
  select(-data) %>%
  mutate(
    id_class = map(id_class, ~paste0(.x, collapse = ";"))
  ) %>%
  # 这一步很重要，必须改变数据框的属性类别!!!
  unnest(cols = c(num_class, num_students)) %>%
  arrange(strat) 
...
dt_sample1 %>%
  ...
  formatRound(columns = ~prob_1_stage, digits = 4)
```
...
dt_sample2 %>%
  ...
  formatRound(columns = ~prob_2_stage, digits = 4) 
```
...
dt_sample2 %>%
  janitor::clean_names() %>%
  select(strat, class, class_size, prob_2_stage) %>%
  distinct() %>%
  janitor::adorn_totals(,,, name = "合计",
                        contains("class_size")) %>%
  datatable(
    caption = "第2阶段样本结果（概率分布）",
    options = list(
      dom = "t", pageLength = 8)
  ) %>%
  formatRound(columns = ~prob_2_stage, digits = 4)
...
dt_sample3 %>%
  ...
  formatRound(columns = ~prob_3_stage+prob, digits = 4)
```
...
dt_sample3 %>%
  janitor::clean_names() %>%
  #select(strat, class, class_size,  prob_3_stage) %>%
  group_by(strat, class, class_size) %>%
  dplyr::summarise(
    num_student=n(),
    prob_3_stage = max(prob_3_stage)) %>%
  distinct() %>%
  janitor::adorn_totals(,,, name = "合计",
                        contains(c("class_size","num_student"))) %>%
  datatable(
    caption = "第3阶段样本结果（概率分布）",
    options = list(
      dom = "t", pageLength = 8)
  ) %>%
  formatRound(columns = ~prob_3_stage, digits = 4)
```
...
```{r}
include_graphics(here("pic/chpt02-non-probability-sampling-2.png"))
```
