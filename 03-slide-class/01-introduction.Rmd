...
```{r}
eggs <- as_tibble(read.xlsx("../data/extra/Table-1-1-eggs.xlsx"))
eggs %>%
  select(STATE, Y1, X1) %>%
...
```{r}
steel <- as.tibble(read.xlsx("../data/extra/Table-1-2-steel.xlsx"))
steel_wide <-steel %>%
...
```{r, out.width= '650px'}
include_graphics("../pic/chpt01-inferential-stat-sampling.png")
```
...
```{r, fig.cap= "一份样本数据"}
include_graphics("../pic/chpt01-curve-points.png", dpi=200)
```
...
include_graphics("../pic/chpt01-curve-L.png", dpi=200)
```
...
include_graphics("../pic/chpt01-curve-U.png", dpi=200)
```
...
include_graphics("../pic/chpt01-curve-S.png", dpi=200)
```
...
```{r}
include_graphics("../pic/chpt01-data-purpose-clearity-message.png")
```
