...
```{r}
load(here("data-analysis//berkeley.Rdata"))
...
```{r}
include_graphics(here("pic/chpt01-believe-bias.png"))
```
...
g <- grViz("digraph flowchart {
      ...
      [5]: '连续变量\\n(Continuous variable)'
      ")
g %>% export_svg %>% charToRaw %>% rsvg %>% png::writePNG(here("pic/flowchart-variables.pdf"))
```
...
g <- grViz("digraph flowchart {
      ...
      [5]: '连续数据\\n(Continuous data)'
      ")
g %>% export_svg %>% charToRaw %>% rsvg %>% png::writePNG(here("pic/flowchart-data.pdf"))
```
...
```{r, fig.cap= "DIKW"}
include_graphics(here("pic/chpt01-data-info-DIKW.png"),dpi ="120")
```
...
.pull-right[
![](../pic/chpt01-bigdata-60-seconds.gif)
]
...
g <- grViz("digraph flowchart {
      ...
      ")
g
#g %>% export_svg %>% charToRaw %>% rsvg %>% png::writePNG(here("pic/flowchart-data-scale.pdf"))
```
...
```{r,fig.cap=""}
include_graphics(here("pic/nominal-show.png"))
```
--
...
nominal <- as_tibble(
  list(
    name= c("Tom","Jenny","Lilei", "Hanmeimei","Dawei"),
    gender= c("M","F", "M", "F", "M"),
    color =c("brown", "blonde", "black", "black", "yellow"),
    area = c("North", "East", "South","West", "Center")))%>%
  add_column(id = 1:nrow(.), .before = "name")

nominal %>%
  datatable(caption ="") %>%
  formatStyle(columns = 1:5, target = "cell",Color = "black")
```
...
```{r, fig.cap= ""}
include_graphics(here("pic/ordinal-show.png"))
```
...
```{r, fig.cap= ""}
include_graphics(here("pic/ordinal-show2.png"))
```
...
.footnote[**说明**：H表示Happy；V表示very；U表示unhappy。三个字母可以组合表达不同含义。]
...
```{r}
scale_smry <- read.xlsx2(here("data-analysis/chpt01-scale-variable.xlsx"),1,startRow = 2) %>%
  as_tibble()
scale_smry %>%
...
include_graphics(here("pic/chpt01-1-data-series-M1.png"), dpi=120)
```
...
```{r}
eggs <- as_tibble(read.xlsx(here("data/extra/Table-1-1-eggs.xlsx")))
eggs %>%
  select(STATE, Y1, X1) %>%
...
```{r}
steel <- as_tibble(read.xlsx(here("data/extra/Table-1-2-steel.xlsx")))
steel_wide <-steel %>%
...
```{r}
include_graphics(here("pic/chpt01-stat-methology.png"),dpi=150)
```
...
```{r}
include_graphics(here("pic/chpt01-descriptive-stat.jpg"), dpi = 100)
```
...
```{r}
include_graphics(here("pic/chpt01-inferential-stat.png"), dpi = 100)
```
...
```{r, out.width= '650px'}
include_graphics(here("pic/chpt01-inferential-stat-sampling.png"))
```
...
```{r, fig.cap= "一份样本数据"}
include_graphics(here("pic/chpt01-curve-points.png"), dpi=200)
```
...
```{r, fig.cap= "A同学的视界：一条直线"}
include_graphics(here("pic/chpt01-curve-L.png"), dpi=200)
```
...
```{r, fig.cap= "B同学的视界：一条抛物线"}
include_graphics(here("pic/chpt01-curve-U.png"), dpi=200)
```
...
```{r, fig.cap= "C同学的视界：一条S型曲线"}
include_graphics(here("pic/chpt01-curve-S.png"), dpi=200)
```
...
```{r}
include_graphics(here("pic/chpt01-data-purpose-clearity-message.png"))
```
...
- 技能1：前沿阅读：学术期刊academic journal（[西农电子期刊数据库](https://lib.nwsuaf.edu.cn/dzzy/qwdzqk/index.htm)；开源期刊库[arXiv.org](https://arxiv.org/)）、RSS订阅（[统计学之都](https://cosx.org/)；[R-bloggers](https://www.r-bloggers.com/)）、[google学术](https://scholar.google.com/schhp?hl=zh-CN&as_sdt=0,5)...
...
> 技能：大学生创新创业训练、开源数据库（国内微观调研数据库、国外[google dataset search](https://datasetsearch.research.google.com/)）、问卷星、Chrome开发工具、HTML标记语言、CSS语言、Regex语言、docker操作、Selenium/RSelenium
...
- 技能：SQL、DBI、[RSQLite](http://dept.stat.lsa.umich.edu/~jerrick/courses/stat701/notes/sql.html)；git语言及[GitHub自动化托管](https://github.com/)。
...
- 技能：Hadoop、Machine Learning and AI、交互制图技术（[R Graph Gallery](https://www.r-graph-gallery.com/interactive-charts.html)）、[shinny APP](https://shiny.rstudio.com/)、books（[Interpretable Machine Learning](https://christophm.github.io/interpretable-ml-book/)）
...
**航班延误背后的故事**：纽约繁忙的天空。[数据来源nycflights13](https://github.com/hadley/nycflights13)
