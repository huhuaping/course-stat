...
```{r}
worker_tbl <- tibble(output = 5:10,
       workers = c(8,12,19,35,25,6))

worker_cumsum <- worker_tbl %>% 
  add_column(group = paste0("G", 1:nrow(.)), .before = "output")%>%
  mutate(cumsum = cumsum(workers)) %>%
  janitor::adorn_totals("row") %>%
  mutate(cumsum = ifelse(group =="Total", NA, cumsum ))

n <- sum(worker_tbl$workers)
```
...
**案例说明**：某工厂共有`r n`个工人，全体工人的日产量（output）经过分组统计后（
$G1 \sim G6$），各组工人人数（workers）的数据如下表所示。请计算中位数是什么？
...
**案例说明**：某工厂共有`r n`个工人，全体工人的日产量（X）经过分组统计后（
...
$\mathbf{QD} =Q_{3}-Q_{1} =92.96-72.4 =20.56$。这**表明**：有一半工人的日产量分布在70.4
