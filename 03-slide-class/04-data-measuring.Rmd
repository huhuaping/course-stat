...
```{r}
include_graphics("../pic/chpt04-rel-structure-demo1.jpg")
```
...
```{r}
include_graphics("../pic/chpt04-rel-structure-gdp.jpg")
```
...
sat_cumsum %>%
  #select(-cumsum ) %>%
  kable(align = "c", booktabs =T, caption = "较小累积频次") %>%
  kable_styling(full_width = T) %>%
  row_spec(c(0,6), bold = T, color = "black", background = "gray")
```
...
hours_bin <- hours_df %>%
  ...
  group_by(X) %>% 
  summarize(f=n())  %>%
  mutate(X =str_replace(X,"\\(0", "\U300C 5"))  %>%
  mutate(X =str_replace(X,"Inf", "30")) %>%
  ungroup() %>%
  mutate(X = mgsub::mgsub(X, "(])","\U300D" )) %>%
...
```{r, out.height= "250px"}
include_graphics("../pic/chpt04-variance-view2.png")
```
